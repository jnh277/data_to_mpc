...
x_star = np.random.uniform(-0.5,1.0, T+N)
x_star = np.reshape(x_star, (-1,10))
x_star[:,1:] = np.atleast_2d(x_star[:,0]).T * np.ones((1,9))
...
for t in tqdm(range(T),desc='Simulating system, running hmc, calculating control'):
    ...
    # measure y_t
    x[t+1] = ssm(x[t], u[t], a=a_true[t]) + q_true * np.random.randn()
    ...
    stan_data = {
        'N': min(t+1, window),
        'y': y[max(0, t+1-window):t+1],
        'u': u[max(0, t+1-window):t+1],
        ...
        'prior_state_mu': x_prior_mu,
        'prior_state_std': x_prior_std,
    }
    ...
    result = solve_chance_logbarrier(np.zeros((1, N)), cost, gradient, hessian, ut, xt, theta, w,
                                     np.reshape(x_star[t:t+N],(1,N)), sqc, src,
...
plt.plot(x_star,linestyle='--',color='g',linewidth=2.,label='target')
...
plt.ylabel('a')
plt.plot(a_true,linestyle='--',color='k',linewidth=2.,label='true')
