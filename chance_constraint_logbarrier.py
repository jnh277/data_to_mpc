...
def chance_constraint(x, s, x_ub, gamma, delta):    # upper bounded chance constraint on the state
    return jnp.mean(expit((x_ub + s - x) / gamma), axis=0) - delta     # take the sum over the samples (M)
...
# test
z0 = jnp.hstack([jnp.zeros((N-1)), 5.0])
...
z = np.hstack([np.zeros((N-1)), 5.0]) # only using one slack variable for all constraints right now
mu = 1e4
gamma = 1.0
args = (device_put(ut), device_put(xt), device_put(x_star), device_put(a),
        device_put(b), device_put(w), device_put(qc), device_put(rc),
