...
model_name = 'single_state_non_affine'
path = 'stan/'
...
def simulate(xt, u, w, theta):
    ...
    for k in range(N):
        x = index_update(x, index[:, :, k+1], a * x[:, :, k] + (0.1 * x[:, :, k]) * u[:, k] + w[:, :, k])
    return x[:, :, 1:]
...
for t in tqdm(range(T),desc='Simulating system, running hmc, calculating control'):
    ...
    stan_data = {
        ...
        'y': y[:t+1],
        'u': u[:t+1],
        'prior_mu': np.array([0.8, 0.1, 0.1]),
        'prior_std': np.array([0.2, 0.2, 0.2]),
        'prior_state_mu': 0.5,
        'prior_state_std': 0.2,
        'nu':nu_true
    }
    ...
    result = solve_chance_logbarrier(0.5*np.ones((1, N)), cost, gradient, hessian, ut, xt, theta, w, x_star, sqc, src,
